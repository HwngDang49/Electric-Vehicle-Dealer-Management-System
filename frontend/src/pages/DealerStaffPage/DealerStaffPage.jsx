...
const DealerStaffPage = () => {
  ...
  const loadOrders = async () => {
    try {
      ...
      const stats = {
        ordersToday: transformedOrders.filter(o => 
        ...
        appointmentsToday: transformedOrders.filter(o => {
          ...
          return new Date(o.scheduledDeliveryDate).toISOString().split("T")[0] === today;
        }).length,
        deliveredOrders: transformedOrders.filter(o => 
      ...
      try {
        ...
        const customers = customersResponse.data?.value || customersResponse.data?.data || customersResponse.data || [];
        stats.newCustomers = customers.filter(c => {
          if (!c.createdAt && !c.CreatedAt) return false;
          return new Date(c.createdAt || c.CreatedAt).toISOString().split("T")[0] === today;
        }).length;
      } catch (error) {
  ...
  const handleUpdateOrderStatus = (orderId, newStatus, newStatusType, vin = null) => {
    setOrders((prev) =>
      prev.map((order) =>
        order.id === orderId
          ? {
          ...
          <>
            <div className="stats-grid">
              ...
              <div className="stat-card">
                ...
                <div className="stat-number">{dashboardStats.appointmentsToday}</div>
              </div>
              <div className="stat-card">
                <h3>Xe đã giao</h3>
                <div className="stat-number">{dashboardStats.deliveredOrders}</div>
              </div>
              <div className="stat-card">
