...
const ContractView = ({
  ...
  // Validate số tiền cọc không được phép lớn hơn tổng giá trị đơn
  const validateDepositAmount = useCallback(
    (depositAmount, showToastError = false) => {
      const trimmedAmount = depositAmount?.toString().trim() || "";
      ...
      const cleanedAmount = trimmedAmount.replace(/\./g, "").replace(/,/g, "");

      // validate người dùng nhập khác số
      if (!/^\d+$/.test(cleanedAmount)) {
        const errorMsg = "Số tiền đặt cọc chỉ được chứa số!";
        setDepositAmountError(errorMsg);
        if (showToastError) {
          showToast("error", errorMsg);
        }
      ...
      // validate không cho phép số 0 ở đầu
      if (cleanedAmount.length > 1 && cleanedAmount.startsWith("0")) {
        const errorMsg = "Số tiền đặt cọc không được bắt đầu bằng số 0";
        setDepositAmountError(errorMsg);
        if (showToastError) {
          showToast("error", errorMsg);
        }
      ...
      const depositValue = parseFloat(cleanedAmount);

      if (isNaN(depositValue) || depositValue <= 0) {
        const errorMsg = "Số tiền đặt cọc phải lớn hơn 0!";
        setDepositAmountError(errorMsg);
        if (showToastError) {
          showToast("error", errorMsg);
        }
      ...
      if (isNaN(orderTotalAmount)) {
        const errorMsg = "Giá trị đơn hàng không hợp lệ!";
        setDepositAmountError(errorMsg);
        if (showToastError) {
          showToast("error", errorMsg);
        }
  ...
  useEffect(() => {
    if (initialToastMessage) {
      showToast(
        initialToastMessage.type || "success",
        initialToastMessage.message
      );
  ...
  const handleFileChange = async (event) => {
    ...
    try {
      ...
      const response = await apiClient.post(
        API_ENDPOINTS.FILES.UPLOAD_CONTRACT,
        formData,
        {
          headers: {
  ...
  const handleMarkAsSigned = async () => {
    try {
      ...
      showToast(
        "success",
        `Hợp đồng đã được ký thành công! Thời gian: ${formattedDate}`
      );
    ...
    <div className="contract-view-app">
      {toast &&
        ReactDOM.createPortal(
          <div
            className={`contract-toast ${
              toast.type === "error" ? "contract-toast-error" : ""
            }`}
            style={{ zIndex: 99999 }}
          >
          ...
          <div className="contract-body">
            {!hasContract && !order.hasContract ? (
              ...
              <div className="no-contract-section">
                ...
                <p>
                  Đơn hàng này chưa có hợp đồng. Bạn có thể tạo hợp đồng mới.
                </p>
                <button
                        ...
                        <input
                          ...
                          value={
                            contractData.signedAt
                              ? formatDate(contractData.signedAt)
                              : ""
                          }
                          placeholder="—"
                          readOnly
                    ...
                    <button
                      ...
                      onClick={async () => {
                        try {
                          ...
                          if (
                            !contractData.fileUrl ||
                            contractData.fileUrl.trim() === ""
                          ) {
                            showToast(
                              "error",
                              "Vui lòng upload file hợp đồng trước khi tạo hợp đồng!"
                            );
                            setLoading(false);
                            return;
                          }
                          ...
                          const response = await apiClient.post(
                            API_ENDPOINTS.ORDERS.CREATE_CONTRACT(
                              order.backendId
                            ),
                            {
