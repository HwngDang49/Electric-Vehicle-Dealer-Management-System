...
const PricebookDetailModal = ({ pricebookId, onClose, onUpdate }) => {
  ...
  const validateEditForm = () => {
    ...
    if (editData.effectiveTo && editData.effectiveFrom >= editData.effectiveTo) {
  ...
  const getProductName = (productId) => {
    ...
    return product ? `${product.name || product.model}` : `Product #${productId}`;
  };
  ...
  return (
    <div className="modal-overlay" onClick={onClose}>
      <div className="product-detail-modal" onClick={(e) => e.stopPropagation()} style={{ maxWidth: '1200px', width: '95%', padding: '0' }}>
        <div className="modal-content" style={{
            ...
            {isEditing ? (
                ...
                <div className="form-row">
                  <div className="form-group">
                    ...
                    {editErrors.effectiveFrom && (
                      <span className="error-text">{editErrors.effectiveFrom}</span>
                    )}
                  </div>
                  ...
                  <div className="form-group">
                    ...
                    {editErrors.effectiveTo && (
                      <span className="error-text">{editErrors.effectiveTo}</span>
                    )}
                  </div>
                </div>
              ...
              <div className="detail-grid">
                <div className="detail-item">
                  ...
                  <span style={{ fontWeight: '600', fontSize: '15px' }}>{pricebook?.name}</span>
                </div>
                ...
                <div className="detail-item">
                  ...
                  <span>{pricebook?.effectiveTo ? formatDate(pricebook?.effectiveTo) : "Không giới hạn"}</span>
                </div>

                <div className="detail-item">
                  <label>Số sản phẩm</label>
                  <span style={{ fontWeight: '600', fontSize: '18px', color: '#3498db' }}>{items.length}</span>
                </div>
          ...
          <div className="detail-section">
            <div className="section-header" style={{
              ...
              <h3 style={{ margin: 0, fontSize: '18px', fontWeight: '600', color: '#2c3e50' }}>
                Sản Phẩm Trong Bảng Giá ({items.length})
              </h3>
              {!showAddItem && (
                <button 
                  className="edit-toggle-btn" 
            ...
            {showAddItem && (
              <div style={{ 
                ...
                <h4 style={{ marginBottom: '15px' }}>Thêm Sản Phẩm Mới</h4>
                ...
                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '15px', marginTop: '15px' }}>
                  <div className="form-group">
                    ...
                    <input
                      ...
                      onChange={(e) => {
                        setNewItem(prev => ({ ...prev, msrpPrice: e.target.value }));
                        setAddItemError("");
                      }}
                      min="0"
                      step="1000000"
                  ...
                  <div className="form-group">
                    ...
                    <input
                      ...
                      onChange={(e) => {
                        setNewItem(prev => ({ ...prev, floorPrice: e.target.value }));
                        setAddItemError("");
                      }}
                      min="0"
                      step="1000000"
            ...
            {items.length === 0 ? (
              <div style={{ textAlign: 'center', padding: '40px', color: '#6c757d' }}>
                ...
                <p style={{ fontSize: '14px' }}>Click "Thêm sản phẩm" để bắt đầu</p>
              </div>
            ) : (
              <div style={{ overflowX: 'auto' }}>
                <table className="detail-table" style={{ 
                  width: '100%', 
                  ...
                  <tbody>
                    {items.map((item) => (
                      <tr key={item.pricebookItemId || item.id}>
                        ...
                        }}>{getProductName(item.productId)}</td>
                        <td style={{ 
                          textAlign: 'right', 
                          fontWeight: '600', 
                          fontSize: '15px',
                        ...
                        <td style={{ 
                          ...
                          <button
                            ...
                            onClick={() => handleRemoveItem(item.pricebookItemId || item.id)}
                            disabled={loading}
                            title="Xóa sản phẩm"
                            style={{
                              background: 'none',
