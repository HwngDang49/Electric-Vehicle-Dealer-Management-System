...
const CreatePromotionModal = ({ onClose, onSuccess, onError }) => {
  ...
  }, [formData.dealerId, formData.fundedBy]);

  const loadDealers = async () => {
    try {
      const response = await dealerApiService.getDealers({ Status: "Live" });
  ...
  const loadProducts = async () => {
    try {
      ...
      // ✅ Handle PagedResult format from backend (similar to PromotionDetailModal)
      const paged = response?.data ?? response;
      const fetchedProducts = Array.isArray(paged) ? paged : paged?.items ?? [];
  ...
  const loadBranches = async () => {
    try {
      const response = await branchApiService.getBranches({ Status: "Active" });
      // ✅ Handle PagedResult format from backend (similar to PromotionDetailModal)
      const paged = response?.data ?? response;
      const fetchedBranches = Array.isArray(paged) ? paged : paged?.items ?? [];
    ...
    <div className="admin-create-promotion-modal-app">
      <div className="modal-overlay" onClick={handleClose}>
        <div
          ...
          style={{ maxWidth: "1400px", width: "95%", maxHeight: "90vh" }}
        >
                ...
                <div className="form-group">
                  ...
                  <CustomDropdown
                    ...
                    options={[
                      ...
                      ...(Array.isArray(dealers) ? dealers : []).map(
                        (dealer) => ({
